...
export function preprocessDino(imgElement) {
...
export function preprocessFace(imgElement, box) {
  const canvas = document.createElement("canvas");
  canvas.width = 160;
  canvas.height = 160;
  const ctx = canvas.getContext("2d");
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (!loadingBar || !loadingText || !btn) {
    console.log(
      "Model loading elements not found on this view. Initialization skipped.",
    );
    return;
  }
  ...
  try {
    ...
    const facenetBuffer = await fetchWithProgress(
    ...
    faceNetSession = await InferenceSession.create(facenetBuffer, {
      executionProviders: ["webgpu", "wasm"],
    });
    ...
    console.error("Failed to initialize vector neural models:", error);
    loadingText.textContent = "Error loading models. Check console logs.";
  }
});
